 ...
 */
 ...
 SYSimulator.MODEL_URL = 'http://210.75.252.106:7080/svn/frank1115/kml/smart.kmz';  
 SYSimulator.TICK_SIM_MS = 66;
 //public vars
 SYSimulator.prototype.totalTime = 0;
 SYSimulator.prototype.totalDistance = 0;
 SYSimulator.prototype.currentSpeed = 0;
 SYSimulator.prototype.currentLoc = null;
 
 //class SYSimulator
 function SYSimulator(ge, path, opt_opts) {
  this.ge = ge;
  this.path = path;
...
SYSimulator.prototype.finishInitUI_ = function(kml, opt_cb) {
  ...
  //initialize car pos
  this.drive_(this.path[0].loc, this.geHelpers_.getHeading(this.path[0].loc, this.path[1].loc));
 ...
 */
SYSimulator.prototype.drive_ = function(loc, heading) {
 ...
 */
SYSimulator.prototype.getTurnToDirection_ = function(heading1, heading2) {
 ...
 */
SYSimulator.prototype.moveToPointDriving_ = function(loc, heading) {
  var oldLa = this.ge.getView().copyAsLookAt(
      //this.ge.ALTITUDE_RELATIVE_TO_GROUND);
	  this.ge.ALTITUDE_ABSOLUTE);
...
SYSimulator.prototype.tick_ = function() {
  ...
  this.totalTime += SYSimulator.TICK_SIM_MS * this.options.speed / 1000.0;
